...
func TestGetGridFilePath(t *testing.T) {
    _ = chdirTemp(t)
    p, err := GetGridFilePath()
    if err != nil {
        t.Fatalf("GetGridFilePath error: %v", err)
    }
    cwd, err := os.Getwd()
    if err != nil {
        t.Fatalf("getwd: %v", err)
    }
    want := filepath.Join(cwd, GridFileName)
    if p != want {
        t.Fatalf("unexpected path: got %s, want %s", p, want)
    }
}
