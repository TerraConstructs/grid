...
3. *** Epic Creation**: From spec.md, create an epic in Beads Tools for the feature using feature name from plan.md.
   - Use the Beads MCP `create` tool to create one top-level issue of type `epic`
...
5. **Generate tasks.md**: Use `.specify.specify/templates/tasks-template.md` as structure, fill with:
   ...
   - Phase 1: Setup tasks (project initialization) create a `feature`-type issue using the MCP `create` tool
   ...
   - Structure:
     ...
     - Beads commands to filter each group:
       ```bash
       beads - list (MCP)(label: "spec:<slug>", label: "phase:setup")
       beads - ready (MCP)(label: "spec:<slug>")
       ```
...
5. **Ordering**:
   ...
   - Final Phase: Polish & Cross-Cutting Concerns
   - **DO NOT generate a linear checklist** — use Beads MCP to build a **task graph**
...
## Example Beads MCP Calls

### Epic

```bash
beads - create (MCP)(title: "Login Feature", description: "...", issue_type: "epic", labels: ["spec:006-login-auth", "component:webapp"], priority: 1, workspace_root: "$WORKSPACE")
```

### Feature

```bash
beads - create (MCP)(title: "Setup Phase", description: "...", issue_type: "feature", deps: ["parent-child:grid-epic-id"], labels: ["spec:006-login-auth", "phase:setup", "component:infra"], priority: 1, workspace_root: "$WORKSPACE")
```

### Task

```bash
beads - create (MCP)(title: "Add React LoginForm", description: "...", issue_type: "task", deps: ["parent-child:grid-feature-id"], labels: ["spec:006-login-auth", "story:US1", "component:webapp"], priority: 2, workspace_root: "$WORKSPACE")
```
