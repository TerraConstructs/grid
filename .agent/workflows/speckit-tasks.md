...
3. *** Epic Creation**: From spec.md, create an epic in Beads Tools for the feature using feature name from plan.md.
   - Use the Beads CLI `bd create` command to create one top-level issue of type `epic`
...
5. **Generate tasks.md**: Use `.specify.specify/templates/tasks-template.md` as structure, fill with:
   ...
   - Phase 1: Setup tasks (project initialization) create a `feature`-type issue using the `bd create` CLI
   ...
   - Structure:
     ...
     - Beads commands to filter each group:
       ```bash
       beads - `bd list --label "spec:<slug>" --label: "phase:setup" --limit 5`
       beads - `bd ready --limit 5`
       ```
