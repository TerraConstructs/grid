...
This feature adds comprehensive authentication (AuthN), authorization (AuthZ), and role-based access control (RBAC) to Grid's API server, protecting both the Connect RPC Control Plane (state/dependency/policy APIs) and the Terraform HTTP Backend Data Plane (/tfstate endpoints). The implementation uses:

- **Casbin** for policy-based authorization with Chi middleware integration
- **OIDC** for SSO authentication (Keycloak dev, Azure Entra ID prod)
- **go-oidc** library for JWT validation and JWKS handling
- **OAuth2 Client Credentials** flow for service accounts
- **PKCE + Loopback redirect** for CLI authentication
- **Group-to-Role Mapping** for enterprise SSO integration (IdP manages group membership, Grid manages role assignments)

Key architectural decisions:
1. Casbin enforces permissions via casbin/chi-authz middleware on Chi router with union (OR) semantics
2. JWT tokens validated against IdP JWKS (no custom signing)
3. Database stores: roles, user-role assignments, group-role mappings, service accounts, sessions, casbin_rule (Casbin policies)
4. Label-scoped access enforced via go-bexpr expressions evaluated at enforcement time using custom bexprMatch function
5. Three default roles seeded: service-account, platform-engineer, product-engineer
6. No scope intersection logic needed - Casbin's default union semantics handle multiple roles automatically
7. msales/casbin-bun-adapter shares existing *bun.DB connection pool (no separate DB connection)
...
- casbin/casbin/v2 - Policy enforcement engine
- casbin/chi-authz - Chi router middleware
...
- coreos/go-oidc/v3 - OIDC client library
...
│   │   ├── oidc.go              # OIDC client wrapper (go-oidc)
│   │   ├── casbin.go            # Casbin enforcer initialization + bexprMatch function
│   │   ├── jwt.go               # JWT validation (JWKS fetch/verify)
...
│   │   ├── authn.go             # Authentication middleware (JWT validation + dynamic grouping)
...
1. **Casbin Integration with Chi Router**
   - Library: github.com/casbin/chi-authz
   - Pattern: Initialize enforcer → attach middleware → extract subject from context
...
3. **CLI Authentication Flow**
   - Pattern: PKCE + Loopback redirect (OAuth2 Authorization Code Flow)
   - Libraries evaluated:
     - golang.org/x/oauth2 (standard, no loopback helper)
     - github.com/nirasan/go-oauth-pkce-code-verifier (PKCE generation)
     - Custom: Embed httptest.Server for localhost callback
   - Port handling: Random free port via net.Listen(":0")
...
5. **JWT Validation**
   - Library: github.com/coreos/go-oidc/v3/oidc
   - Process: Fetch JWKS → verify signature → validate claims (iss, aud, exp)
   - No custom JWT signing (IdP is issuer)
...
The /tasks command will generate tasks from Phase 1 artifacts in the following order:
...
3. Seed migration for default Casbin policies (insert into casbin_rule with go-bexpr expressions)
   - **Seed data**: research.md §7, plan.md §310-323 (default role policies)
...
4. Config struct updates (OIDC issuer, client ID, JWKS URLs, Casbin model path, groups_claim_field, groups_claim_path)
   ...
   - **Claim config**: CLARIFICATIONS.md §1 (lines 72-90), research.md §8 (JWT claim extraction)
...
9. OIDC client initialization (go-oidc wrapper)
   - **Pattern**: research.md §2 (lines 112-165, discovery + JWKS caching)
   - **Provider setup**: research.md §2 (Keycloak dev, Azure Entra ID prod)
10. JWT validation function (JWKS fetch + verify)
    - **Validation flow**: research.md §6 (lines 313-375, signature + claims validation)
    - **Claims**: research.md §6 (iss, aud, exp, sub)
11. JWT claim extraction helper (ExtractGroups function with mapstructure for nested claims)
    - **Flat array extraction**: research.md §8 (lines 619-686, direct array handling)
    - **Nested extraction**: research.md §8 (lines 688-765, mapstructure pattern)
...
17. Authentication middleware (JWT extraction + validation) [P]
...
20. Login handler (GET /auth/login - redirect to IdP) [P]
    - **Flow**: research.md §2 (lines 112-165, OIDC initiation)
    - **State generation**: Use crypto/rand for CSRF protection
21. Callback handler (GET /auth/callback - exchange code) [P]
    - **Exchange**: research.md §2 (code→token exchange via OIDC provider)
...
34. Device code flow implementation (PKCE + loopback)
    - **Flow**: research.md §3 (lines 167-214, PKCE + loopback redirect pattern)
    - **Port handling**: research.md §3 (net.Listen(":0") for random port)
    - **Libraries**: golang.org/x/oauth2 + crypto/rand for PKCE verifier
...
36. Login command (gridctl login)
    - **Flow**: research.md §3 (initiate device code flow, wait for callback)
...
38. Auth status command (gridctl auth status)
    - **Display**: User identity, roles, token expiry
    - **RPC**: Call GetEffectivePermissions (task #31)
